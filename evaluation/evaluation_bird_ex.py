...
from evaluation.clean import clean_and_process_sql
...
def execute_model(predicted_sql, ground_truth, db_place, idx, meta_time_out):
    try:
        ...
        import traceback
        traceback.print_exc()  # 输出异常堆栈，方便排查
        result = [(f'error',)]
        res = 0
    # print(result)
    ...
    if mode == 'gpt':
        ...
        for idx, sql_str in sql_data:  # .items()
            if type(sql_str) == str:
                ...
                sql, db_name = " ", "thrombosis_prediction"
...
if __name__ == '__main__':
    ...
    args_parser.add_argument('--predicted_sql_json_path',default=os.path.join(os.path.abspath(os.path.join('..')),'outputs','bird','predict.json'), type=str)
    args_parser.add_argument('--ground_truth_sql_path', default=os.path.join(os.path.abspath(os.path.join('..')),'data', 'bird', 'dev_gold.sql'), type=str)
    args_parser.add_argument('--data_mode', type=str, default='dev', choices=['train', 'dev', 'test'])
    args_parser.add_argument('--db_root_path', default=os.path.join(os.path.abspath(os.path.join('..')),'data', 'bird', 'dev_databases'), type=str)
    args_parser.add_argument('--num_cpus', type=int, default=12)
    args_parser.add_argument('--meta_time_out', type=float, default=30.0)
    args_parser.add_argument('--mode_predict', type=str, default='gpt')
    args_parser.add_argument('--diff_json_path',type=str,default=os.path.join(os.path.abspath(os.path.join('..')),'data', 'bird', 'dev.json'))


    args = args_parser.parse_args()

    exec_result = []
    data, message = clean_and_process_sql(args.predicted_sql_json_path,args.predicted_sql_json_path)
    print(message)
    # generate pred sqls:
