...
def llm_api_func(prompt, model=MODEL_NAME, temperature=0):
    global MODEL_NAME
    ...
    return text,prompt_token,response_token       # 返回模型生成的文本
...
def safe_call_llm(input_prompt, **kwargs) -> str:
    ...
    for i in range(5):
        try:
            if log_path is None:
                print(input_prompt)
                sys_response, prompt_token, response_token = llm_api_func(input_prompt)
                #print(f"\n sys_response: \n{sys_response}")
