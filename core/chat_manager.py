...
from core.agents import Preprocessor, Generator, Refiner
from core.const import MAX_ROUND, SYSTEM_NAME, PREPROCESSOR_NAME, GENERATOR_NAME, REFINER_NAME
...
class ChatManager(object):
    def __init__(self, data_path: str, tables_json_path: str, log_path: str, model_name: str, dataset_name:str, lazy: bool=False):
        ...
        self.ping_network()
        self.chat_group = [
            Preprocessor(data_path=self.data_path, tables_json_path=self.tables_json_path, model_name=self.model_name, dataset_name=dataset_name, lazy=lazy),
    ...
    def start(self, user_message: dict):
        ...
        if user_message['send_to'] == SYSTEM_NAME:  # in the first round, pass message to prune
            user_message['send_to'] = PREPROCESSOR_NAME
...
if __name__ == "__main__":
    test_manager = ChatManager(data_path="../data/bird/database",
                               log_path="",
