...
def run_batch(dataset_name, input_file, output_file, db_path, tables_json_path, start_pos=0, log_file=None,
    ...
    with open(output_file, 'a+', encoding='utf-8') as fp:
        ...
        for cur_idx, item in tqdm(enumerate(batch), total=total_num):
            ...
            if dataset_name == "spider":
                ...
                user_message = init_bird_message(idx, item, db_path=db_path,
                                                 use_gold_schema=use_gold_schema)  # imitate user send a question to system
            try:
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--input_file', type=str, default=os.path.join('.', 'data', 'bird', 'dev.json'), help='path to dataset input')
    # parser.add_argument('--db_path', type=str, default=os.path.join('.', 'data', 'bird', 'dev_databases'), help='path to databases in dataset')
    # parser.add_argument('--tables_json_path', type=str, default=os.path.join('.', 'data', 'bird', 'dev_tables.json'), help='path to tables.json')
    # parser.add_argument('--output_file', type=str, default=os.path.join('.', 'outputs', 'bird', 'output_bird.json'), help='path to predicted output')
    # parser.add_argument('--log_file', type=str, default=os.path.join('.', 'outputs', 'bird', 'log.txt'), help='path to log file if needed')
    ...
    parser.add_argument('--dataset_mode', type=str, default='dev', choices=['train', 'dev', 'test'],
                        help='dataset mode')
    parser.add_argument('--input_file', type=str, default=os.path.join('.', 'data', 'spider', 'dev.json'),
                        help='path to dataset input')
    parser.add_argument('--db_path', type=str, default=os.path.join('.', 'data', 'spider', 'database'),
                        help='path to databases in dataset')
    parser.add_argument('--tables_json_path', type=str, default=os.path.join('.', 'data', 'spider', 'tables.json'),
                        help='path to tables.json')
    parser.add_argument('--output_file', type=str, default=os.path.join('.', 'outputs', 'spider', 'output.json'),
                        help='path to predicted output')
    parser.add_argument('--log_file', type=str, default=os.path.join('.', 'outputs', 'spider', 'log.txt'),
